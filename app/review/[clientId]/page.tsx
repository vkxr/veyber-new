...
export default function ReviewPage() {
  ...
  const generateReview = async (clientInfo: any) => {
    setLoading(true);
    setReviewText("");
    setError(null);
    try {
      const res = await fetch("/api/generate-review", {
        ...
        body: JSON.stringify({
          ...
          customPrompt: clientInfo.customPrompt,
        }),
      });
  ...
  const handleCopyAndRedirect = async () => {
    if (!reviewText || !clientData?.googleReviewLink) return;
    ...
    try {
      ...
      setTimeout(() => {
        setCopied(false);
        window.open(clientData.googleReviewLink, "_blank", "noopener,noreferrer");
      }, 800);
    } catch (err) {
  ...
  if (error && !clientData) {
    return (
      <div className="min-h-screen bg-[#0F1021] text-white flex items-center justify-center p-4">
        <div className="bg-[#1C1D36] border border-[#2F3151] rounded-2xl p-6 sm:p-8 shadow-2xl text-center max-w-md w-full">
        ...
        <div className="bg-[#1C1D36] border border-[#2F3151] rounded-2xl p-6 sm:p-8 shadow-2xl overflow-hidden transition-all duration-300">
          ...
          <div className="text-center mb-6">
            <h1 className="text-2xl sm:text-3xl font-bold font-headline mb-3 tracking-tight text-white">
              {clientData ? `Review ${clientData.businessName}` : 'Leave a Quick Review'} ⭐
            </h1>
          ...
          <div className="mb-6 relative">
            {loading ? (
              ...
              <div className="space-y-4 fade-in animate-in duration-500 relative">
                ...
                <textarea
                  ...
                  className="w-full h-32 sm:h-36 bg-[#0B0C1A] border border-[#2F3151] rounded-xl p-4 text-sm sm:text-base text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors resize-none cursor-text"
                  placeholder="Your review here..."
                />
                
                {/* Regenerate Button */}
                <button 
                  onClick={() => generateReview(clientData)} 
                  className="absolute bottom-4 right-4 text-gray-500 hover:text-white transition-colors p-2 rounded-full cursor-pointer hover:bg-[#2F3151]"
